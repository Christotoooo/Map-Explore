...
public class HexGraph<E extends HexGridCell> extends Graph<E>  {
    ...
    public static class HexEdge<E> extends Graph.Edge<E>{
        ...
        public String toTxt(int width) {
        	String s = type.toTxt() + " ";
            int from_xHex, from_yHex;
            int to_xHex, to_yHex;
        	if (from.getData() instanceof TerrainCell) {
        		from_xHex = ((TerrainCell)from.getData()).getHexX();
        		from_yHex = ((TerrainCell)from.getData()).getHexY();
        	}
        	else {
        		return "";
        	}
        	if (to.getData() instanceof TerrainCell) {
        		to_xHex = ((TerrainCell)to.getData()).getHexX();
        		to_yHex = ((TerrainCell)to.getData()).getHexY();
        	}
        	else {
        		return "";
        	}
        	if (type == null || type == ConnType.kDefault) {
        		s = "";
        	} else if (type.isBarrier()) {	// Need to check type, here! Maybe later.
                s += String.valueOf(from_xHex) + " " + String.valueOf(from_yHex) + " ";
        		switch(HexDir.inferDir(new Pair(to_xHex-from_xHex, to_yHex-from_yHex, 'B'))) {
                case upLeft:
                	s += "ul";
                	break;
                case downLeft:
        		...
        		s += String.valueOf(from_xHex)+" "+String.valueOf(from_yHex)+" "
        			+ String.valueOf(to_xHex)+" "+String.valueOf(to_yHex);
        	}
        	return s;
        }
    }
